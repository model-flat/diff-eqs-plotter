...
var
  ...
  begin
    if Assigned(Parser) then
      Result := not Assigned(Parser.FindFunction(S)) and not Assigned(Parser.FindType(S))
    else
      Result := not Assigned(Data.NameList) or (Data.NameList.List.IndexOf(S) < 0);
  end;
...
begin
  ...
  if not Result then
  begin
    S := PrecedingText(Text, Index);
    Result := Check;
    if Result then
    begin
      S := FollowingText(Text, Index + Count);
      Result := Check;
      if Result then
      begin
        GetFlagArray(S, ltChar, Data^, FlagArray);
        try
          J := Index + Count;
          for I := Low(FlagArray) to High(FlagArray) do
          begin
    ...
    try
      ...
      while I <= Count do
        ...
        begin
          ...
          begin
            Inc(J);
            if (J < Count) and (Text[J] = DecimalSeparator) and
