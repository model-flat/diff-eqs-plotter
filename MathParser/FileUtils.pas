...
procedure ForceDirectories(const FileName: string);
...
begin
  ...
  if FilePath <> '' then SysUtils.ForceDirectories(FilePath);
end;
...
begin
  ...
  try
    ...
    try
      Search(Path, FileList, True, FolderList);
      for I := 0 to FileList.Count - 1 do DeleteFile(FileList[I]);
      for I := 0 to FolderList.Count - 1 do
      begin
        SetFileAttributes(PChar(FolderList[I]), 0);
        RemoveDirectory(PChar(FolderList[I]));
      end;
    finally
      FolderList.Free;
    end;
...
var
  Handle: THandle;
begin
  Handle := Windows.CreateFile(PChar(FileName), GENERIC_READ, FILE_SHARE_READ or
