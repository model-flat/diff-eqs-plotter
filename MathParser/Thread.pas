...
{ Thread                                                                  }
{                                                                         }
{ Copyright (c) 2008 Pisarev Yuriy (mail@pisarev.net)                     }
{                                                                         }
{ *********************************************************************** }
...
type
  ...
  public
    ...
    function ForceStop(Timeout: Integer): Boolean; overload; virtual;
    function ForceStop: Boolean; overload; virtual;
...
begin
  ...
  begin
    ...
    ForceStop(FStopTimeout);
  end;
  FFatalException.Free;
  inherited;
end;

function TThread.ForceStop: Boolean;
begin
  Result := FStarted and TerminateThread(FHandle, 0);
  if Result then
  begin
    FStarted := False;
    FForceStopped := True;
...
function TThread.ForceStop(Timeout: Integer): Boolean;
begin
  Result := FStarted;
  if Result then
  begin
    ...
    if not Result then Result := ForceStop;
  end;
end;
...
begin
  if Value <> FSuspended then
    if not FStarted then FSuspended := Value
    else
  ...
  begin
    ...
    if FSuspended then
      if FStackSize > 0 then
        FHandle := BeginThread(nil, FStackSize, @ThreadMethod, Pointer(Self), CREATE_SUSPENDED or
...
function TThread.WaitFor(Timeout: Longword): Boolean;
begin
