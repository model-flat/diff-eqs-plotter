...
type
  ...
  protected
    ...
    function GetNumeration(Index: Integer): string; virtual; abstract;
    function GetPositiveChar: Char; virtual; abstract;
    procedure SetNegativeChar(const Value: Char); virtual; abstract;
    procedure SetNumeration(Index: Integer; const Value: string); virtual; abstract;
  ...
  public
    ...
    function ConvertFrom(Value: string; Index: Integer): Int64; virtual; abstract;
    function ConvertTo(Value: Int64; Index: Integer): string; virtual; abstract;
    property Count: Integer read GetCount;
    property Numeration[Index: Integer]: string read GetNumeration write SetNumeration; default;
  published
  ...
  protected
    ...
    function GetNumeration(Index: Integer): string; override;
    function GetPositiveChar: Char; override;
    procedure SetNegativeChar(const Value: Char); override;
    procedure SetNumeration(Index: Integer; const Value: string); override;
  ...
  public
    ...
    function ConvertFrom(Value: string; Index: Integer): Int64; override;
    function ConvertTo(Value: Int64; Index: Integer): string; override;
  published
...
begin
  if Check(Value) then Result := FList.Add(Value)
  else Result := -1;
end;
...
var
  I: Integer;
begin
  Result := (Length(Value) > 1) and not Contains(Value, FNegativeChar) and
    not Contains(Value, FPositiveChar);
  if Result then
...
function TNumeration.ConvertFrom(Value: string; Index: Integer): Int64;
var
  Flag: Boolean;
  S: string;
  I, J, K, L: Integer;
begin
  ...
  Result := 0;
  Flag := CutText(Value, FNegativeChar);
  S := FList[Index];
  J := Length(Value);
  L := Length(S);
  for I := 1 to J do
...
function TNumeration.ConvertTo(Value: Int64; Index: Integer): string;
var
  Negative: Boolean;
  S: string;
  I: Int64;
...
procedure TNumeration.Delete(Index: Integer);
begin
  inherited;
  FList.Delete(Index);
end;
...
function TNumeration.GetNumeration(Index: Integer): string;
begin
  Result := FList[Index];
end;
...
procedure TNumeration.SetNumeration(Index: Integer; const Value: string);
begin
  FList[Index] := Value;
end;
