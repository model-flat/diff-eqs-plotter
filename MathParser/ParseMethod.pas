...
type
  ...
  private
    FVariableList: TFlexibleList;
  protected
    procedure Clear; virtual;
    property VariableList: TFlexibleList read FVariableList write FVariableList;
  public
...
const
  ...
  VariableError = 'Variable "%s" not found';
...
uses
  Math, MemoryUtils, NumberConsts, NumberUtils, ParseConsts, ParseErrors, ParseManager,
  Parser, ParseUtils, ThreadUtils, ValueConsts, ValueErrors, ValueUtils;
...
var
  ...
  Value: PValue;
begin
  for I := 0 to FVariableList.List.Count - 1 do
  begin
    Value := PValue(FVariableList.List.Objects[I]);
...
begin
  inherited;
  FVariableList := TFlexibleList.Create(nil);
end;
...
var
  ...
  I: Integer;
begin
  Check(AFunction, ParameterArray, DeleteMinParameterCount, DeleteMaxParameterCount);
  ...
  BFunction := AParser.FindFunction(Parameter.Text);
  if Assigned(BFunction) and (BFunction.Method.MethodType = mtVariable) then
  begin
    I := FVariableList.List.IndexOf(BFunction.Name);
    if I < 0 then AssignInteger(Result, AParser.FalseValue)
    else begin
      AParser.DeleteFunction(BFunction.Handle^);
      Dispose(PValue(FVariableList.List.Objects[I]));
      FVariableList.List.Delete(I);
      AssignInteger(Result, AParser.TrueValue);
    end
  end
  else begin
    if AsBoolean(Parser, ParameterArray, BIndex, True) then
      raise ParseErrors.Error(VariableError, [Parameter.Text]);
...
var
  ...
  Variable: PFunctionVariable;
begin
  Check(AFunction, ParameterArray, GetMinParameterCount, GetMaxParameterCount);
  Parameter := GetParameter(FParser, ParameterArray[AIndex]);
  Check(FParser, AFunction, AIndex, Parameter.TypeHandle, True);
  BFunction := TParser(FParser).FindFunction(Parameter.Text);
  if Assigned(BFunction) and (BFunction.Method.MethodType = mtVariable) then
  begin
    Variable := @BFunction.Method.Variable;
    case Variable.VariableType of
...
begin
  ...
  Check(FParser, AFunction, AIndex, Parameter.TypeHandle, True);
  S := Parameter.Text;
  if Assigned(AParser.FindFunction(S)) then
    AssignInteger(Result, AParser.FalseValue)
  else begin
    Parameter := GetParameter(FParser, ParameterArray[BIndex]);
    New(Value);
    ...
    AParser.AddVariable(S, Value^, AsBoolean(Parser, ParameterArray, CIndex, False),
      True, ValueType);
    FVariableList.List.AddObject(S, TObject(Value));
    AssignInteger(Result, AParser.TrueValue);
  end;
end;
...
var
  ...
  Variable: PFunctionVariable;
begin
  Check(AFunction, ParameterArray, SetMinParameterCount, SetMaxParameterCount);
  Check(FParser, AFunction, BIndex, ParameterArray[BIndex].TypeHandle, False);
  Parameter := GetParameter(FParser, ParameterArray[AIndex]);
  Check(FParser, AFunction, AIndex, Parameter.TypeHandle, True);
  BFunction := TParser(FParser).FindFunction(Parameter.Text);
  ...
  begin
    ...
    if AsBoolean(Parser, ParameterArray, CIndex, True) then
      raise ParseErrors.Error(VariableError, [Parameter.Text]);
    AssignInteger(Result, TParser(FParser).FalseValue);
  end;
end;
...
function TMathMethod.DegreeMethod(const AFunction: PFunction; const AType: PType;
  const LValue, RValue: TValue): TValue;
begin
  AssignDouble(Result, Power(Convert(LValue, vtDouble).Float64,
