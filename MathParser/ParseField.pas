...
{ ParseField                                                              }
{                                                                         }
{ Copyright (c) 2010 Pisarev Yuriy (mail@pisarev.net)                     }
{                                                                         }
{ *********************************************************************** }
...
uses
  Windows, SysUtils, Classes, Variants, DB, EventManager, Calculator, FlexibleList,
...
uses
  NotifyManager, TextConsts, TextUtils, ValueConsts, ValueUtils;
...
var
  I: Integer;
begin
  if Available(FCalculator) then FCalculator.BeginUpdate;
  try
    for I := FList.List.Count - 1 downto 0 do Delete(I);
    FList.List.Clear;
  finally
    if Available(FCalculator) then FCalculator.EndUpdate;
  end;
end;
...
begin
  ...
  FCalculator.EventManager.Add(FFunctionHandle, FFunctionName, FPriority, Custom);
end;
...
begin
  ...
  if Result then
  begin
    FCalculator.BeginUpdate;
    try
      Result := Assigned(DataSet);
      if Result then AOwner := DataSet.Owner
      else AOwner := nil;
                ...
                begin
                  ...
                  begin
                    ...
                    try
                      ...
                      FCalculator.Parser.AddFunction(S, Item.FunctionHandle, fkHandle,
                        FunctionMethod(False, False, 0), False, False, vtDouble);
