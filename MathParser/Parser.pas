...
type
  ...
  published
    property FalseValue: Integer read FFalseValue write FFalseValue default Ord(False);
    property TrueValue: Integer read FTrueValue write FTrueValue default Ord(True);
...
begin
  for I := Low(TCacheType) to High(TCacheType) do
  begin
    FCacheArray[I].LightCache.CacheType := Value;
    FCacheArray[I].HeavyCache.CacheType := Value;
  end;
end;
...
begin
  for I := Low(TCacheType) to High(TCacheType) do
  begin
    FCacheArray[I].LightCache.NameValueSeparator := FNameValueSeparator;
    FCacheArray[I].HeavyCache.NameValueSeparator := FNameValueSeparator;
  end;
end;
{$ENDIF}
...
begin
  ...
  try
    ...
    InternalAddFunction(DeleteFunctionName, FDeleteHandle, fkMethod,
      FunctionMethod(FMethod.DeleteMethod, DeleteMinParameterCount, pkReference),
      False, False);
    InternalAddFunction(GetFunctionName, FGetHandle, fkMethod,
      FunctionMethod(FMethod.GetMethod, GetMinParameterCount, pkReference),
      False, False);
    InternalAddFunction(SetFunctionName, FSetHandle, fkMethod,
      FunctionMethod(FMethod.SetMethod, SetMinParameterCount, pkReference),
      False, False);
...
begin
  ...
  try
    ...
    InternalAddFunction(IsZeroFunctionName, FIsZeroHandle, fkMethod, FunctionMethod(FMethod.IsZeroMethod, IsZeroMaxParameterCount),
      True, True, vtByte);
    InternalAddFunction(IfFunctionName, FIfHandle, fkMethod, FunctionMethod(FMethod.IfMethod, IfParameterCount, pkReference),
      True, True, vtByte);
...
function TCustomParser.InternalAddFunction(const AName: string;
  ...
  Optimizable, Whole: Boolean; ReturnType: TValueType): Boolean;
begin
  Result := InternalAddFunction(MakeFunction(AName, Handle, ReturnType, Kind,
    Method, Optimizable, Whole));
end;
